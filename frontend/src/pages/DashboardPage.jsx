  ...
  if (renderError || !data) {
    return (
      <div className="min-h-[80vh] flex flex-col items-center justify-center p-8 text-center">
        <div className="bg-red-500/10 border border-red-500/20 p-6 rounded-3xl max-w-md text-slate-100">
           <h2 className="text-red-400 font-bold mb-2">Dashboard Initialization Failed</h2>
           <p className="text-slate-400 text-sm mb-4">{renderError || "No data received from intelligence engine. Please ensure your Supabase tables are created."}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8 text-slate-100">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-lg lg:col-span-2">
            <div className="flex justify-between items-center mb-6">
              ...
              <span className="text-xs font-bold text-slate-500 bg-slate-800 px-2 py-1 rounded text-slate-100">DAILY RESOLUTION</span>
            </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-lg flex flex-col text-slate-100">
                 ...
                 {recentTx.map((t, i) => (
                    <div key={i} className="flex justify-between items-center group cursor-default">
                       <div className="flex items-center gap-3">
                          ...
                          <div>
                             <p className="font-bold text-slate-200 text-sm group-hover:text-blue-400 transition-colors">{t.merchant || 'Unknown'}</p>
                             <p className="text-[10px] text-slate-500 uppercase tracking-wider font-bold mt-0.5">{t.category || 'Other'}</p>
                          </div>
                       </div>
                       <div className="text-right">
                          ...
                          <p className="text-[10px] text-slate-600 font-medium mt-0.5">{new Date(t.date || Date.now()).toLocaleDateString()}</p>
                       </div>
                    </div>
                 ))}
              </div>
...
function MetricCard({ title, value, icon, subValue, trend, status }) {
   ...
   return (
      <div className={`bg-slate-900 border ${isHighRisk ? 'border-red-500/50' : 'border-slate-800'} rounded-2xl p-6 shadow-lg shadow-black/20 transition-all hover:border-slate-700 relative overflow-hidden group`}>
         ...
         <div className="flex justify-between items-start mb-4 text-slate-100">
